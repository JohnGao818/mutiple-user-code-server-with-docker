...
exports.logToolFlowDiagnostics = logToolFlowDiagnostics;
...
const consts_3 = require("../vision/consts");
...
function logToolFlowDiagnostics({ requestKind, tools, messagesFiltered, preflight, activatePreflight, nextRound, initialResponseNotice, }) {
    if (!(0, config_1.getDebugLoggingEnabled)()) {
        return;
    }
    const activateToolCount = tools?.reduce((count, tool) => count + (tool.name.startsWith(consts_2.ACTIVATE_TOOL_PREFIX) ? 1 : 0), 0) ?? 0;
    if (preflight === 'skipped' && !messagesFiltered && activateToolCount === 0) {
        return;
    }
    let message = `[tool-flow] preflight=${preflight}` +
    ...
    logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, message));
}
class DefaultCacheDiagnosticsRecorder {
    previousCacheTraces = new Map();
    lastCacheTracesByScope = new Map();
    lastCacheTrace;
    requestId = 0;
    isEnabled() {
    ...
    beginRequest(options) {
        const requestKind = options.requestKind ??
            (0, routing_1.classifyDeepSeekRequest)({
        ...
        const immediateTraceKeyChanged = previousImmediateCacheTrace?.cacheTraceKey !== undefined &&
            previousImmediateCacheTrace.cacheTraceKey !== cacheTrace.cacheTraceKey;
        const sameImmediateComparisonScope = previousImmediateCacheTrace !== undefined &&
        ...
        const traceKeyChangeComparison = previousImmediateCacheTrace && immediateTraceKeyChanged && sameImmediateComparisonScope
            ? compareCacheTraceSnapshots(previousImmediateCacheTrace, cacheTrace)
            : undefined;
        const skippedImmediateFallback = previousImmediateCacheTrace && immediateTraceKeyChanged && !sameImmediateComparisonScope
            ? previousImmediateCacheTrace
            : undefined;
        const scopedTraceKeyChangeComparison = !traceKeyChangeComparison &&
            ...
            : undefined;
        const visionResolution = summarizeVisionResolution(options.inputMessages, options.resolvedMessages, options.visionModelId, options.visionProxySource);
        logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] ${formatCacheTraceSnapshot(cacheTrace)}`));
        logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] request ` +
            (0, routing_1.formatModelFields)(options.vscodeModelId, options.request.model) +
        ...
        if (shouldLogHostPromptTrace(hostPromptTrace)) {
            logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] ${formatHostPromptTrace(hostPromptTrace)}`));
        }
        const vscodeMessageTrace = formatVscodeMessageTrace(options.inputMessages);
        if (vscodeMessageTrace) {
            logger_1.logger.debug((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] vscodeMsgs ${vscodeMessageTrace}`));
        }
        for (const detailLine of formatCacheTraceDetailLines(cacheTrace)) {
            const message = (0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] ${detailLine}`);
        ...
        if (visionTrace) {
            logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] ${visionTrace}`));
        }
        if (cacheTraceComparison) {
            logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] ${formatCacheTraceComparison(cacheTraceComparison)}`));
            for (const detailLine of formatCacheTraceComparisonDetailLines(cacheTraceComparison)) {
                logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] ${detailLine}`));
            }
            for (const warning of getCacheTraceComparisonWarnings(cacheTraceComparison)) {
                logger_1.logger.warn((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] ${warning}`));
            }
        }
        if (traceKeyChangeComparison && previousImmediateCacheTrace) {
            logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] ${formatCacheTraceKeyChangeComparison(previousImmediateCacheTrace.cacheTraceKey, cacheTrace.cacheTraceKey, traceKeyChangeComparison)}`));
            for (const detailLine of formatCacheTraceComparisonDetailLines(traceKeyChangeComparison)) {
                logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] cacheTraceKeyChanged ${detailLine}`));
            }
            for (const warning of getCacheTraceComparisonWarnings(traceKeyChangeComparison)) {
                logger_1.logger.warn((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] cacheTraceKeyChanged fallback diff: ${warning}`));
            }
        }
        if (skippedImmediateFallback) {
            logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] comparisonScopeChanged` +
        ...
        if (scopedTraceKeyChangeComparison && previousScopedCacheTrace) {
            logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] sameScope ${formatCacheTraceKeyChangeComparison(previousScopedCacheTrace.cacheTraceKey, cacheTrace.cacheTraceKey, scopedTraceKeyChangeComparison)}`));
            for (const detailLine of formatCacheTraceComparisonDetailLines(scopedTraceKeyChangeComparison)) {
                logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] sameScope cacheTraceKeyChanged ${detailLine}`));
            }
            for (const warning of getCacheTraceComparisonWarnings(scopedTraceKeyChangeComparison)) {
                logger_1.logger.warn((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] sameScope cacheTraceKeyChanged fallback diff: ${warning}`));
            }
        }
        ...
        for (const infoLine of getCacheTraceInfoLines(cacheTrace)) {
            logger_1.logger.info((0, routing_1.formatRequestLogLine)(requestKind, `[cache-trace #${requestId}] ${infoLine}`));
        }
        return new ActiveCacheDiagnosticsRun(this, requestId, cacheTrace, {
            vscodeModelId: options.vscodeModelId,
            apiModelId: options.request.model,
            requestKind,
        }, cacheTraceComparison ?? traceKeyChangeComparison ?? scopedTraceKeyChangeComparison, cacheTraceComparison
...
function getCacheTraceStoreKey(snapshot) {
    return `${snapshot.requestKind}:${snapshot.cacheTraceKey}`;
}
function getCacheTraceScopeKey(snapshot) {
...
class ActiveCacheDiagnosticsRun {
    ...
    constructor(recorder, requestId, snapshot, usageLogContext, resultComparison, prefixLabel) {
        this.recorder = recorder;
        this.requestId = requestId;
        this.snapshot = snapshot;
        this.usageLogContext = usageLogContext;
        this.resultComparison = resultComparison;
        this.prefixLabel = prefixLabel;
    }
    onDone(info) {
        if (info.emittedToolCalls > 0 || info.trailingToolResults > 0) {
            logger_1.logger.info((0, routing_1.formatRequestLogLine)(this.snapshot.requestKind, `[cache-trace #${this.requestId}] stream done reasoningTextChars=${info.reasoningTextChars}` +
    ...
    onUsage(usage, charsPerToken) {
        ...
        if (this.resultComparison) {
            ...
            logger_1.logger.info((0, routing_1.formatRequestLogLine)(this.snapshot.requestKind, `[cache-trace #${this.requestId}] result cacheRate=${hitRate}%` +
    ...
    onCancellationTokenRequested() {
        ...
        logger_1.logger.info((0, routing_1.formatRequestLogLine)(this.snapshot.requestKind, `[cache-trace #${this.requestId}] cancellation token requested; aborting stream`));
    }
    onReplayMarkerReport(info) {
        logger_1.logger.info((0, routing_1.formatRequestLogLine)(this.snapshot.requestKind, `[cache-trace #${this.requestId}] ${formatReplayMarkerReport(info)}`));
    }
}
class NoopCacheDiagnosticsRun {
    usageLogContext;
    constructor(usageLogContext) {
        this.usageLogContext = usageLogContext;
    }
    onDone(_info) { }
    onCancellationTokenRequested() { }
...
function logUsage(usage, charsPerToken, context, requestId) {
    const cacheHit = usage.prompt_cache_hit_tokens ?? 0;
    const cacheMiss = usage.prompt_cache_miss_tokens ?? 0;
    logger_1.logger.info((0, routing_1.formatRequestLogLine)(context.requestKind, `tokens${requestId ? ` #${requestId}` : ''}: ` +
        (0, routing_1.formatModelFields)(context.vscodeModelId, context.apiModelId) +
...
function summarizeVisionResolution(inputMessages, resolvedMessages, visionModelId, visionProxySource) {
    const stats = {
    ...
    for (const [index, message] of inputMessages.entries()) {
        ...
        if (imageParts > 0) {
            ...
            const newFailures = Math.max(0, countLiteral(resolvedText, consts_3.IMAGE_DESCRIPTION_UNAVAILABLE) -
                countLiteral(inputText, consts_3.IMAGE_DESCRIPTION_UNAVAILABLE));
...
function countImageDataBytes(message) {
    let bytes = 0;
...
function formatVisionTrace(stats, pipelineStats) {
    ...
    const note = inputImageParts + toolImageParts === 0 && stats.historyDescriptionMessages > 0
        ? ' note=history-only'
        : '';
    const parts = [
        `vision mode=${mode}`,
    ...
    if (pipelineStats && hasVisionPipelineActivity(pipelineStats)) {
        parts.push(`toolResultImages=${toolImageParts}`, `toolResults=${pipelineStats.tool.resultsWithImages}`, `toolResultBytes=${toolImageBytes}`, `toolForwarded=${toolForwardedImageParts}`, `toolDropped=${toolDroppedImageParts}`, `current=${pipelineStats.currentImageMessages}`, `generated=${pipelineStats.generatedImageMessages}`, `replayed=${pipelineStats.replayedImageMessages}`, `omitted=${pipelineStats.omittedImageMessages}`);
...
function formatVisionMimeStats(stats) {
    return stats
        .map((item) => `${JSON.stringify(item.mimeType)}:parts=${item.imageParts}:bytes=${item.imageBytes}`)
        .join(',');
}
function hasVisionPipelineActivity(stats) {
    ...
    return (stats.imageHandlingMode !== 'none' ||
...
function formatVisionModel(stats, pipelineStats) {
...
function createCacheTraceSnapshot(request, inputMessages = [], requestKind = (0, routing_1.classifyDeepSeekRequest)({ request, inputMessages })) {
...
function summarizeMessage(message, index, followsToolResult) {
    ...
    const unableImageCount = countLiteral(contentText, consts_3.IMAGE_DESCRIPTION_UNAVAILABLE);
...
function summarizeStats(messages, toolCount) {
    ...
    for (const message of messages) {
        ...
        if (contentText.includes(consts_3.IMAGE_DESCRIPTION_UNAVAILABLE)) {
...
function toDiagnosticContentText(content) {
    if (typeof content !== 'object' || !Array.isArray(content)) {
        return (0, content_1.deepSeekContentToText)(content, { separator: '\n' });
    }
    const parts = [];
...
function toDiagnosticMessageFingerprint(message) {
...
function formatDiagnosticImageUrlSummary(url) {
...
function summarizeImageUrlForDiagnostics(url) {
...
function estimateBase64DecodedBytes(base64Payload) {
