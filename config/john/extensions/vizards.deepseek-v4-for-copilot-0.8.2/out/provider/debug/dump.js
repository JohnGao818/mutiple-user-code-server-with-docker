...
function dumpProviderInput(options) {
    ...
    const requestKind = options.requestKind ??
        (0, routing_1.classifyProviderRequest)({
            messages: options.messages,
            tools: options.requestOptions.tools,
        });
    const context = createDumpContext(options.globalStorageUri, options.segment, 'deepseek-provider-input', (providerInputDumpCounter += 1), requestKind);
    const paths = createProviderInputDumpPaths(context);
    const toolSummary = summarizeTools(options.requestOptions.tools);
    enqueueDumpWrite((0, routing_1.formatRequestLogLine)(requestKind, 'providerInputDump'), async () => {
...
function dumpDeepSeekRequest(request, options) {
    ...
    const toolSummary = summarizeTools(options.requestOptions.tools);
    enqueueDumpWrite((0, routing_1.formatRequestLogLine)(requestKind, 'requestDump'), async () => {
        await (0, promises_1.mkdir)(context.root, { recursive: true });
        ...
        if (msg0 && paths.msg0) {
            await writeTextFile(paths.msg0, (0, content_1.deepSeekMessageToText)(msg0, { includeImageUrls: true, separator: '\n' }));
        }
        await writeDumpObservation(options.globalStorageUri, createDumpObservation({
...
function createDumpContext(globalStorageUri, segment, prefix, seq, requestKind) {
...
function summarizeDeepSeekSystemPrompt(messages) {
    ...
    return createSystemPromptSummary(0, message.role, (0, content_1.deepSeekMessageToText)(message, { includeImageUrls: true, separator: '\n' }), customizations);
}
...
function summarizeDeepSeekCustomizations(messages) {
    ...
    let latestUserHasCustomizationsUpdate = false;
    for (const [index, message] of messages.entries()) {
        const text = (0, content_1.deepSeekMessageToText)(message, { includeImageUrls: true, separator: '\n' });
        customizationsUpdateCountInHistory += countLiteral(text, '<customizationsUpdate>');
...
function getStringSetting(section, key) {
    const value = vscode_1.default.workspace.getConfiguration(section).get(key);
...
function formatFileUri(fsPath) {
    return vscode_1.default.Uri.file(fsPath).toString();
}
function logProviderInputDump(options, paths, toolSummary, requestKind) {
    const systemPromptSummary = summarizeVscodeSystemPrompt(options.messages);
    logger_1.logger.debug((0, routing_1.formatRequestLogLine)(requestKind, `providerInputDump written: ${formatDumpSegment(options.segment)}` +
        ` ${(0, routing_1.formatModelFields)(options.modelInfo.id)}` +
...
function logRequestDump(request, options, paths, requestJsonLength, requestKind) {
    const systemPromptSummary = summarizeDeepSeekSystemPrompt(request.messages);
    logger_1.logger.debug((0, routing_1.formatRequestLogLine)(requestKind, `requestDump written: ${formatDumpSegment(options.segment)}` +
        ` ${(0, routing_1.formatModelFields)(options.vscodeModelId, request.model)}` +
...
function formatSettingValue(value) {
    ...
    if (value === 'unknown')
        return 'unknown';
    return (0, json_1.safeStringify)(value);
}
