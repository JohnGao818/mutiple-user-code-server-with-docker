...
function streamChatCompletion({ prepared, progress, token, initialResponseNotice, getCharsPerToken, setCharsPerToken, }) {
    ...
    const cancelListener = (0, debug_1.observeCancellationToken)(token, prepared.cacheDiagnostics);
    return prepared.client
        ...
        onError: (error) => {
            throw (0, client_1.createUserFacingError)(error);
        },
        onDone: () => {
...
function reportReplayMarker(prepared, progress, state, trigger) {
    ...
    try {
        ...
        logger_1.logger.warn((0, routing_1.formatRequestLogLine)(prepared.requestKind, 'Failed to report replay marker'), error);
    }
}
...
function reportCopilotContextUsage(progress, usage, requestKind) {
    const data = {
    ...
    try {
        ...
        logger_1.logger.warn((0, routing_1.formatRequestLogLine)(requestKind, 'Failed to report usage data'), error);
    }
}
